...
export default function Groups() {
  const { groups, branches, levels, options } = useSelector((state) => state);
              ...
              {currentData.map((item, key) => {
                return (
                  <tr key={key}>
                    ...
                    <td data-label="nom complet" className="p-1 ">
                      {levels.find(elem => elem.id == branches.find(elem1 => elem1.id == options.find(elem2 => elem2.id == item.option).branch).level).name}
                    </td>
                    <td data-label="nom complet" className="p-1 ">
                      {options.find(elem => elem.id == item.option).season == 1 ? "1ere année" : "2éme année"}
                    </td>
                    <td data-label="nom complet" className="p-1">
                      {options.find(elem => elem.id == item.option).season == 2? options.find(elem=>elem.id==item.option).name:`----------------------`}
                    </td>
                    <td data-label="nom complet" className="p-1 ">
