                    ...
                    {assignements.filter(item => item.group == student.group).map((item, key) => {
                        return <tr>
                            ...
                            {Array.from({ length: maxControl }, (_, i) => i + 1).map((elem) =>
                                <td key={elem}>{notes && (notes.find(elem1 => elem1.assignement == item.id && elem1.control == elem) ? ("0" + Number(notes.find(elem1 => elem1.assignement == item.id && elem1.control == elem).note).toFixed(2)).slice(-5) : "--")}</td>)}
